...
- **A와 B가 동일 속도(~525 pkt/s 각)로 증가, 차이 상수(2~3)** → **두 망이 각각 전체 프레임을 실어나르는 이중 전송(redundant)** 이 실시간 관측됨. 단 A/B가 **동시 도착이 아니라 스큐(시간차)** 가 있고, 수신 엔드시스템이 **SN 순서로 먼저 온 유효 프레임을 채택하고 늦은 중복을 제거**(first valid wins, SkewMax 창). B는 실질적 **백업 경로**(한 망 장애 대비).
...
| 복제 | 송신 ES가 A·B 이중 전송(백업, 스큐 존재) | generation 노드가 R-TAG 삽입 후 복제 |
| 오버헤드 | 없음 | +6바이트 R-TAG |
