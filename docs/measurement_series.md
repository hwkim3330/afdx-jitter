...
## ★★★ 지터 미시구조 — 240ns 양자 (2026-09-11)
HW 타임스탬프 간격을 이산 레벨로 분해(`quantum_analysis.py`, `results/quantum_levels.json|quantum_hist.png`).

**발견: AFDX 송신 지터는 랜덤 노이즈가 아니라 결정론적 2레벨 양자화.**
- BAG 2ms에서 간격이 **딱 두 값** — 2000093ns(69.6%)·1999855ns(30.4%), 차이 **238ns**.
- 전 BAG(1~16ms)에서 **양자 = 240ns 일정**, 분할비율만 BAG/240ns의 소수부 따라 변함.
- **240ns = 72.0 × (1/300MHz)** — 보드 디바이스트리 `clk=299970000`(300MHz 인터페이스 클록) 방증. FPGA BAG 스케줄러가 **300MHz의 72클록마다 틱**(240ns 격자)하는 것으로 해석.
- 기구: AFDX BAG 단위(10µs)=41.67×240ns(비정수) → 프레임 릴리스가 인접 격자점 ±1양자로 진동 = 유계 결정론적 지터.

**의미**: FPGA 페이싱 지터는 **BAG 무관하게 최악 ±240ns로 유계**(랜덤 누적 없음). AFDX 실시간성 관점에서 이상적 — Max Jitter 예산(µs~수십µs) 대비 훨씬 여유. RMS 110ns는 이 2레벨의 분산일 뿐.
